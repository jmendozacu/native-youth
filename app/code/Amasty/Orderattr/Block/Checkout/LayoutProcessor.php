...
class LayoutProcessor implements \Magento\Checkout\Block\Checkout\LayoutProcessorInterface
{
    ...
    protected function addToShippingStep(&$jsLayout, $attributes)
    {
        ...
        if (count($elements) > 0) {
            ...
            if ($this->customerSession->isLoggedIn() && ($customer->getDefaultShippingAddress() || count($customer->getAdditionalAddresses()) > 0)) {
    ...
    protected function getElementsByAttributes($attributes, $checkoutStepId)
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            $elements['amorderattr_' . $attribute->getAttributeCode()] = $this->attributeMapper->map($attribute);
        }
        return $elements;
    }
