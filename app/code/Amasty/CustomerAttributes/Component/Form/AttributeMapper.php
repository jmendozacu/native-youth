...
class AttributeMapper extends \Magento\Ui\Component\Form\AttributeMapper
{
    ...
    public function map($attribute)
    {
        ...
        if ($attribute->usesSource()) {
            ...
            $meta['options'] = array_values($allOptions);
        }
        ...
        if (isset($meta['frontend_class'])) {
            if ($meta['frontend_class'] == 'validate-length') {
                $rules[$meta['frontend_class']] = 'maximum-length-25';
                $rules['max_text_length'] = 25;
            }
            else{
        ...
        if ($attribute->getFrontendInput() == 'boolean') {
            $meta['options'] = $this->_objectManager->get('Amasty\CustomerAttributes\Block\Data\Form\Element\Boolean')->getValues();
        }
