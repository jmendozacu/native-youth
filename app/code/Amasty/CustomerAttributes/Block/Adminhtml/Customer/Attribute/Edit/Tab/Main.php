...
class Main extends AbstractMain
{
    ...
    protected function _prepareForm()
    {
        ...
        $scopes = array(
            Attribute::SCOPE_STORE => __('Store View'),
            Attribute::SCOPE_WEBSITE => __('Website'),
            Attribute::SCOPE_GLOBAL => __('Global'),
        );
        ...
        $fieldset->addField(
            ...
            [
                ...
                'label' => __('Image Dimentions'),
                'title' => __('Image Dimentions - in pixels like: 30/40 (where 30 - width, 40 - height)'),
                'note' => __('In pixels like: 30/40 (where 30 - width, 40 - height)'),
            ]
        );
        ...
        $fieldset->addField(
            ...
            [
                'name' => 'account_filled',
                'label' => __('Do not Show if Filled'),
                'title' => __('Do not Show if Filled - on the Account Information page on the Frontend'),
        ...
        $fieldset->addField(
            ...
            [
                'name' => 'billing_filled',
                'label' => __('Do not Show if Filled'),
                'title' => __('Do not Show if Filled - on the Billing page during Checkout'),
        ...
        if ($attributeObject->getId()
            ...
            foreach (
                $rewriteAttributeValue[$attributeObject->getAttributeCode()]
                as $field => $value
            ) {
    ...
    protected function _getAttributeObject()
    {
        ...
        if (!$this->_storeManager->isSingleStoreMode()) {
            if(!$attrObject->getData('store_ids')) {
                ...
                $attrObject->setData(
                    'stores', explode(',', $attrObject->getData('store_ids'))
                );
            }
