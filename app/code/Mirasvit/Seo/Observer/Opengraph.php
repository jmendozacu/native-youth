...
class Opengraph implements ObserverInterface
{
    ...
    public function modifyHtmlResponse($e)
    {
        ...
        if (($this->config->getCategoryOpenGraph() && $this->seoData->getFullActionCode() == 'catalog_category_view')
            || ($this->config->isCmsOpenGraphEnabled() && $this->seoData->getFullActionCode() == 'cms_page_view')) {
            $tags[] = $label;
