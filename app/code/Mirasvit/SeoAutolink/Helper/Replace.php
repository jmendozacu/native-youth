...
class Replace extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    protected function replace($replace, $source, $maxReplacements, $replaceKeyword = false, $direct = false)
    {
        ...
        $source = $this->_restoreSurceByTranslationTable($translationTable, $source);

        //reconstruct the original text if was replacement inside block (double check)
        if (strpos($source, 'xkjndsfkjnakcx') !== false && strpos($source, 'cxmkmweof329jc') !== false) {
            $source = $this->_restoreSurceByTranslationTable($translationTable, $source);
        }
