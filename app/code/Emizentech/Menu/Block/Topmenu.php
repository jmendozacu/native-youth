<?php
namespace Emizentech\Menu\Block;
...
class Topmenu extends \Smartwave\Megamenu\Block\Topmenu
{
    ...
    public function getMegamenuHtml()
    {
        ...
        $category_new = $objectManager->get('\Magento\Framework\Registry')->registry('current_category');//get current category
        if($category_new){
        $current_cate_id = $category_new->getId();
        }
        ...
        foreach($categories as $category) {
            ...
            //echo $this->registry->registry('current_category')->getId();
            ...
            if(!$sw_menu_hide_item) {
                ...
                $sub_class = '';
                if($category_new){
                    if($current_cate_id == $category->getId() ){
                    ...
                    foreach($children as $child) {
                        $chile_array[] = $child->getId();
                    
                    }
               
                    if(in_array($current_cate_id, $chile_array)){
                        $sub_class = 'selected';
                ...
                if(count($children) > 0)
                    $item_class .= 'parent ';
                $html .= '<li class="ui-menu-item '.$item_class.$sw_menu_float_type.' parent_'.$category->getId().' '.$selected_Class.' '.$sub_class.'">';
                if(count($children) > 0) {
                    $html .= '<div class="open-children-toggle"></div>';
                }
                $html .= '<a href="'.$this->_categoryHelper->getCategoryUrl($category).'" class="level-top '.$selected_Class.' '.$sub_class.'">';
                if ($sw_menu_icon_img)
