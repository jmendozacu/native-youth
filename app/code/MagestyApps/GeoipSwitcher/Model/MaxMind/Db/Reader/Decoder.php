...
namespace MagestyApps\GeoipSwitcher\Model\MaxMind\Db\Reader;
...
class Decoder
{
        ...
        if ($type == 'extended') {
            ...
            if ($typeNum < 8) {
                throw new LocalizedException(
                    __("Something went horribly wrong in the decoder. An extended type "
    ...
    private function decodeByType($type, $offset, $size)
    {
        ...
        switch ($type) {
            ...
            default:
                throw new LocalizedException(
                    __("Unknown or unexpected type: " . $type)
                );
        }
    }
    ...
    private function verifySize($expected, $actual)
    {
        if ($expected != $actual) {
            throw new LocalizedException(
                __("The MaxMind DB file's data section contains bad data (unknown data type or corrupt data)")
            );
        }
    }
