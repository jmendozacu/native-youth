    ...
    )
    {
        ...
        if ($isEnabled && $isPriceCountdown) {
            ...
            if ($product->getTypeId() == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE) 
            {
                ...
                //var_dump($_children->getId());
                 foreach ($_children as $child)
                 {   
                    
                    $toDate = $child->getSpecialToDate() ? strtotime($child->getSpecialToDate()) : '';
                    if($toDate!= null){ //echo "ok";
                    $result .= '
                    <div class="mb-timecountdown-container timer-category timer-'.$style.'" data-todate="' . $toDate . '" data-prd_id="' . $child->getId() . '">
                        <div class="timer-heading">PRICE COUNTDOWN</div>
                        <div class="timer-countbox" id="price-countdown-'.$child->getId().'"></div>
                    </div>
                    ';
                    }
                 }
